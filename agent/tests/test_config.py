...
def test_loved_ones_are_loaded_as_editorial_anchors() -> None:
    ...
    assert (
        anchors[0].url == "https://www.seangoedecke.com/you-have-to-beat-the-models-at-something/"
    )
    assert anchors[0].tags == ["ai", "decision-making"]


def test_issue_sources_are_in_the_registry() -> None:
    urls = {str(source.feed_url) for source in Settings().registry().sources}
    assert {
