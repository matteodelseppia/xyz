...
type Ref = { sourceId: string; url: string; title: string; text: string; publishedAt: string | null };
type Evidence = {
...
function evidence(value: Ref, kind: Evidence["kind"]): Evidence {
  ...
  return { id, source_id: value.sourceId, url: value.url, title: text(value.title).slice(0, 300), published_at: normalizePublicationDate(value.publishedAt), text: clean, kind };
}

export function unresolvedEvidenceIds(
  updates: Array<{ evidence_ids: string[]; verifications?: Array<{ evidence_ids: string[] }> }>,
  knownIds: ReadonlySet<string>,
): string[] {
  return [...new Set(updates.flatMap(update => [
    ...update.evidence_ids,
    ...(update.verifications ?? []).flatMap(check => check.evidence_ids),
...
export function pageEntries(body: string, source: Source, baseUrl: string): Array<Record<string, unknown>> {
  ...
  for (const match of body.matchAll(/<a\b[^>]*href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi)) {
    ...
    try {
      ...
      if (parsed.origin !== base.origin || parsed.pathname === base.pathname || title.length < 12 || seen.has(url)) continue;
      seen.add(url);
      const value: Ref = { sourceId: source.id, url, title, text: title, publishedAt: null };
      const ev = evidence(value, "feed");
      entries.push({ entry_ref: token(value), evidence_id: ev.id, title, url, published_at: null, description: title, _evidence: ev });
    } catch { /* malformed links are ignored */ }
  }
  return entries;
}

export function publicationDate(body: string): string | null {
  const dateProperty = "(?:article:published_time|datePublished|publish_date|date)";
  const visibleDate = "(\\d{4}-\\d{2}-\\d{2}|(?:Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:t(?:ember)?)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)\\s+\\d{1,2},?\\s+\\d{4}|\\d{1,2}\\s+(?:Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:t(?:ember)?)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)\\s+\\d{4})";
  const candidates = [
    body.match(new RegExp(`<meta\\b[^>]*(?:property|name|itemprop)=["']${dateProperty}["'][^>]*content=["']([^"']+)["']`, "i")),
    body.match(new RegExp(`<meta\\b[^>]*content=["']([^"']+)["'][^>]*(?:property|name|itemprop)=["']${dateProperty}["']`, "i")),
    body.match(/["']datePublished["']\s*:\s*["']([^"']+)["']/i),
    body.match(/<time\b[^>]*datetime=["']([^"']+)["'][^>]*>/i),
    text(body).match(new RegExp(`\\b(?:published|posted|written)(?:\\s+on)?\\s*[:—-]?\\s*${visibleDate}`, "i")),
  ];
  for (const match of candidates) {
    if (!match?.[1]) continue;
    const normalized = normalizePublicationDate(match[1]);
...
const Verification = Type.Object({
  assertion: Type.Union([Type.Literal("heading"), Type.Literal("description"), Type.Literal("why_read")]),
  question: Type.String({ minLength: 1, maxLength: 180 }),
  evidence_ids: Type.Array(Type.String(), { minItems: 1, maxItems: 5 }),
}, { additionalProperties: false });
const PublicationUpdate = Type.Object({
  heading: Type.String({ minLength: 1, maxLength: 100 }),
  description: Type.String({ minLength: 1, maxLength: 360 }),
  why_read: Type.String({ minLength: 1, maxLength: 220 }),
  keywords: Type.Array(Type.String({ minLength: 1, maxLength: 32 }), { minItems: 1, maxItems: 5 }),
  evidence_ids: Type.Array(Type.String(), { minItems: 1, maxItems: 5 }),
  verifications: Type.Array(Verification, { minItems: 3, maxItems: 3 }),
}, { additionalProperties: false });
const PublicationUpdates = Type.Union([
  Type.Array(PublicationUpdate, { minItems: 0, maxItems: 0 }),
  Type.Array(PublicationUpdate, { minItems: 10, maxItems: 10 }),
]);

const ReviewCheck = Type.Object({
  update_index: Type.Integer({ minimum: 0, maximum: 11 }),
  evidence_supported: Type.Boolean(),
  attribution_correct: Type.Boolean(),
  selection_rationale_credible: Type.Boolean(),
  opinion: Type.String({ minLength: 1, maxLength: 280 }),
  uncertainty: Type.Optional(Type.Union([Type.String({ maxLength: 220 }), Type.Null()])),
}, { additionalProperties: false });
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    name: "read_feed", label: "Read source", description: "Read up to 15 recent entries from one configured feed or news page.",
    ...
    async execute(_id, params) {
      ...
      if (/<!DOCTYPE/i.test(fetched.body) && !/<html\b/i.test(fetched.body)) throw new Error("feed DOCTYPE declarations are not permitted");
      ...
      let entries = rawItems.map(item => {
        ...
        const value: Ref = { sourceId: source.id, url: absolute, title, text: description || title, publishedAt };
      ...
      if (!entries.length && /<html\b/i.test(fetched.body)) entries = pageEntries(fetched.body, source, fetched.url);
  ...
  pi.registerTool({
    ...
    async execute(_id, params) {
      ...
      const value = { ...ref, url: fetched.url, text: normalized || ref.text, publishedAt: ref.publishedAt ?? publicationDate(fetched.body) };
      ...
      for (const match of fetched.body.matchAll(/<a\b[^>]*href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi)) {
        ...
        try {
          ...
          links.push({ link_ref: token({ sourceId: ref.sourceId, url, title: text(match[2]), text: "linked page", publishedAt: ref.publishedAt }), label: text(match[2]).slice(0, 120) });
        } catch { /* malformed links are ignored */ }
      }
  ...
  pi.registerTool({
    ...
    async execute(_id, params) {
      ...
      const fetched = await boundedFetch(ref.url);
      const ev = evidence({ ...ref, url: fetched.url, text: text(fetched.body), publishedAt: ref.publishedAt ?? publicationDate(fetched.body) }, "link");
  ...
  if (role === "producer") pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      updates: PublicationUpdates,
      revision_notes: Type.Optional(Type.Union([Type.String({ maxLength: 500 }), Type.Null()])),
    }, { additionalProperties: false }),
    async execute(_id, params) {
      const unknown = unresolvedEvidenceIds(params.updates, knownEvidenceIds);
      if (unknown.length) throw new Error(`unknown evidence IDs: ${unknown.join(", ")}`);
      return { content: [{ type: "text" as const, text: `Submitted ${params.candidate_id}` }], details: { artifact: params }, terminate: true };
    },
  });
  ...
  if (role === "reviewer") pi.registerTool({
    ...
    parameters: Type.Object({ schema_version: Type.Literal(1), candidate_id: Type.String(), approved: Type.Boolean(), findings: Type.Array(Finding, { maxItems: 12 }), checks: Type.Array(ReviewCheck, { maxItems: 12 }), rationale: Type.String({ minLength: 1, maxLength: 500 }) }, { additionalProperties: false }),
    async execute(_id, params) {
